...
async function runTests() {
  ...
  if (panchang) {
    ...
    log('Panchang data not available for Baishakh 1', 'red');
  }

  // Test specific tithi - Purnima
  const purnimaPanchang = getPanchang({ year: 2082, month: 1, day: 29 });
  if (purnimaPanchang) {
    log(`\nBaishakh 29 (Purnima):`, 'yellow');
    log(`  Tithi: ${purnimaPanchang.tithi.name} (${purnimaPanchang.tithi.number})`, 'green');
  }

  // Test 3: Calendar Grid Generation
  subheader('3. Calendar Grid Generation (Ashwin 2082 - Dashain Month)');

  const calendar = await getMonthCalendar(2082, 6, { // Ashwin is month 6
