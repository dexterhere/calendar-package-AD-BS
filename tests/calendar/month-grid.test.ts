...
describe('getMonthDays', () => {
  it('BS 2082 Baishakh (month 1) has 31 days', () => {
    expect(getMonthDays(2082, 1)).toBe(31)
  })

  it('BS 2082 Ashadh (month 3) has 32 days', () => {
    expect(getMonthDays(2082, 3)).toBe(32)
  })

  it('BS 2082 Mangsir (month 8) has 29 days', () => {
    expect(getMonthDays(2082, 8)).toBe(29)
  })

  it('BS 2082 Chaitra (month 12) has 30 days', () => {
...
describe('getMonthCalendar — grid shape', () => {
  it('total days array length is always a multiple of 7 (complete week rows)', () => {
  ...
  it('grid has exactly 35 or 42 days (5 or 6 complete week rows)', () => {
    for (let month = 1; month <= 12; month++) {
      const cal = getMonthCalendar(2082, month)
      expect([35, 42], `BS 2082 month ${month}`).toContain(cal.days.length)
    }
  })

  it('without adjacent days, days.length equals totalDays exactly', () => {
    const cal = getMonthCalendar(2082, 1, { includeAdjacentDays: false })
    expect(cal.days.length).toBe(31)
    expect(cal.days.length).toBe(cal.totalDays)
  })

  it('totalDays field matches the actual month length', () => {
    expect(getMonthCalendar(2082, 1).totalDays).toBe(31)
    expect(getMonthCalendar(2082, 3).totalDays).toBe(32)
    expect(getMonthCalendar(2082, 8).totalDays).toBe(29)
  })
})
...
describe('getMonthCalendar — startWeekday', () => {
  it('BS 2082 Baishakh starts on Monday (startWeekday = 1)', () => {
    // Verified: BS 2082-01-01 = AD 2025-04-14 = Monday
    const cal = getMonthCalendar(2082, 1)
    expect(cal.startWeekday).toBe(1)
  })

  it('BS 2083 Baishakh starts on Tuesday (startWeekday = 2)', () => {
    // Verified: BS 2083-01-01 = AD 2026-04-14 = Tuesday
    const cal = getMonthCalendar(2083, 1)
    expect(cal.startWeekday).toBe(2)
  })

  it('first overflow day count equals startWeekday', () => {
    const cal = getMonthCalendar(2082, 1) // startWeekday = 1
...
describe('getMonthCalendar — isCurrentMonth', () => {
  it('all days with isCurrentMonth=true have the correct year and month', () => {
    const cal = getMonthCalendar(2082, 1)
  ...
  it('overflow days have isCurrentMonth=false', () => {
    const cal = getMonthCalendar(2082, 1)
  ...
  it('count of isCurrentMonth=true days matches totalDays', () => {
    const cal = getMonthCalendar(2082, 1)
    const currentCount = cal.days.filter(d => d.isCurrentMonth).length
    expect(currentCount).toBe(cal.totalDays)
  })

  it('without adjacent days, all days are isCurrentMonth=true', () => {
    const cal = getMonthCalendar(2082, 1, { includeAdjacentDays: false })
...
describe('getMonthCalendar — overflow day content', () => {
  it('pre-month overflow days come from the previous month in correct order', () => {
    const cal = getMonthCalendar(2082, 1) // starts Monday, so 1 overflow day before
  ...
  it('post-month overflow days are the first days of the next month', () => {
    const cal = getMonthCalendar(2082, 1) // 31 current + 1 pre = 32, needs 3 more to reach 35
  ...
  it('year boundary: BS 2082 Chaitra overflow days come from BS 2083 Baishakh', () => {
    const cal = getMonthCalendar(2082, 12)
  ...
  it('year boundary: BS 2083 Baishakh pre-overflow days come from BS 2082 Chaitra', () => {
    const cal = getMonthCalendar(2083, 1)
...
describe('getMonthCalendar — weekday correctness', () => {
  it('consecutive days have consecutive weekdays (wrapping at Saturday → Sunday)', () => {
    const cal = getMonthCalendar(2082, 1)
    for (let i = 1; i < cal.days.length; i++) {
      ...
      const expectedDay = (prev.weekday.en === 'Saturday') ? 'Sunday'
        : { Sunday: 'Monday', Monday: 'Tuesday', Tuesday: 'Wednesday',
            Wednesday: 'Thursday', Thursday: 'Friday', Friday: 'Saturday' }[prev.weekday.en]
      expect(curr.weekday.en, `day ${i}`).toBe(expectedDay)
    }
  })

  it('first day of BS 2082 Baishakh has weekday Monday', () => {
    const cal = getMonthCalendar(2082, 1)
  ...
  it('weekday Nepali names are populated', () => {
    const cal = getMonthCalendar(2082, 1, { includeAdjacentDays: false })
...
describe('getMonthCalendar — AD date correctness', () => {
  it('BS 2082 Baishakh 1 maps to AD 2025-04-14', () => {
    const cal = getMonthCalendar(2082, 1, { includeAdjacentDays: false })
  ...
  it('consecutive days have AD dates exactly 1 day apart', () => {
    const cal = getMonthCalendar(2082, 1)
    for (let i = 1; i < cal.days.length; i++) {
      const prev = cal.days[i - 1]!
      const curr = cal.days[i]!
...
describe('getMonthCalendar — metadata fields', () => {
  it('year and month fields match the input', () => {
    const cal = getMonthCalendar(2082, 6)
    expect(cal.year).toBe(2082)
    expect(cal.month).toBe(6)
  })

  it('monthName is populated in English and Nepali', () => {
    const cal = getMonthCalendar(2082, 1)
    expect(cal.monthName.en).toBe('Baishakh')
    expect(cal.monthName.ne).toBe('बैशाख')
  })

  it('monthName for Chaitra (month 12) is correct', () => {
    const cal = getMonthCalendar(2082, 12)
    expect(cal.monthName.en).toBe('Chaitra')
  })

  it('panchang is null for all days when enrichPanchang is false', () => {
    const cal = getMonthCalendar(2082, 1, { enrichPanchang: false })
    expect(cal.days.every(d => d.panchang === null)).toBe(true)
  })

  it('events is empty array for all days when enrichEvents is false', () => {
    const cal = getMonthCalendar(2082, 1, { enrichEvents: false })
    expect(cal.days.every(d => d.events.length === 0)).toBe(true)
  })

  it('classification is neutral for all days when enrichEvents is false', () => {
    const cal = getMonthCalendar(2082, 1, { enrichEvents: false })
...
describe('getMonthCalendar — isToday', () => {
  it('at most one day has isToday=true across the whole grid', () => {
...
describe('nextMonth', () => {
  ...
  it('wraps from Chaitra (12) to Baishakh (1) of next year', () => {
...
describe('prevMonth', () => {
  ...
  it('wraps from Baishakh (1) to Chaitra (12) of previous year', () => {
...
describe('monthRange', () => {
  ...
  it('returns all months in a year range', () => {
  ...
  it('returns 12 months for a full BS year', () => {
    const result = monthRange(
